...
void run_collapse(double *res_holder,double threshold = 1e15,double delta_ini_fac = 1.0)
{
	...
	double lin_delta,lin_delta_a,lin_delta_rk[5], lin_delta_a_rk[5], lin_acc;
	double lin_delta_i, lin_delta_a_i;
	...
	lin_delta_i = lin_delta_ini_fac*ai;
	lin_delta_a_i = 0.000;
	
	lin_delta = lin_delta_i;
	lin_delta_a = lin_delta_a_i;
	...
	for(a=ai_burn,cntr=0;(a<=a0)&&(delta<threshod);a+=da,++cntr)
	{
		ak = a;
		...
		lin_delta_rk[0] = lin_delta;
		lin_delta_a_rk[0] = lin_delta_a; 
		...
		if((a>ai)&&(burn))
		{
			...
			lin_delta_i = lin_delta;
			lin_delta_a_i = lin_delta_a;			
		...
		for(i=1;i<=4;++i)
		{
			...
			if(theory==0)			
			lin_acc  = cosmo_model_lcdm.lin_delta_aa( ak,  lin_delta_rk[0], lin_delta_a_rk[0]);
			if(theory==1)			
			lin_acc  = cosmo_model_dgp.lin_delta_aa( ak,  lin_delta_rk[0], lin_delta_a_rk[0]);
			if(theory==2)			
			lin_acc  = cosmo_model_bigravity.lin_delta_aa( ak,  lin_delta_rk[0], lin_delta_a_rk[0]);
			...
			lin_delta_a_rk[i] = da*lin_acc;
			lin_delta_rk[i] = da*lin_delta_a_rk[0];
	

			//if(a==ai)
			//printf("acc %.10lf\n",lin_delta_rk[i]);

			lin_delta_rk[0] = lin_delta + rk_coef[i-1]*lin_delta_rk[i];
			lin_delta_a_rk[0] = lin_delta_a + rk_coef[i-1]*lin_delta_a_rk[i];
		...
		lin_delta = lin_delta + (1.0/6.0)*(lin_delta_rk[1]+2.0*lin_delta_rk[2]+2.0*lin_delta_rk[3]+lin_delta_rk[4]);
		lin_delta_a = lin_delta_a + (1.0/6.0)*(lin_delta_a_rk[1]+2.0*lin_delta_a_rk[2]+2.0*lin_delta_a_rk[3]+lin_delta_a_rk[4]);
